        ...
        match expr {
            ...
            UnrolledExpr::BuiltIn(b) => {
                let mel_b = match *b {
                    ...
                    ExpandedBuiltIn::BitAnd(e1,e2) => self.binop(e1,e2, ExpandedBuiltIn::<MelExpr>::BitAnd),
                    ExpandedBuiltIn::BitOr(e1,e2) => self.binop(e1,e2, ExpandedBuiltIn::<MelExpr>::BitOr),
                    ExpandedBuiltIn::BitXor(e1,e2) => self.binop(e1,e2, ExpandedBuiltIn::<MelExpr>::BitXor),
...
pub fn count_insts(e: &MelExpr) -> u16 {
    match e {
        ...
        MelExpr::BuiltIn(b) => match &**b {
            ...
            ExpandedBuiltIn::BitAnd(e1,e2) => 1 + count_insts(&e1) + count_insts(&e2),
            ExpandedBuiltIn::BitOr(e1,e2) => 1 + count_insts(&e1) + count_insts(&e2),
            ExpandedBuiltIn::BitXor(e1,e2) => 1 + count_insts(&e1) + count_insts(&e2),
